# File:     swath_total_protein_approach_calc.R
...
MOL_CONVERSION <- as.double(10^9) # MW (g/mol))*10^9

input_dir <- "input_file"
output_dir <- "output_files"
input_hist_dir <- "raw_input_history"
output_file_prfx <- "TPA-calc_"
...
# Get all samples cols names in df_swath ####################
...
for( j in 1:length(samples_cols)){ # length(samples_cols
  ...
  # load each summit into the sums vector
  sums.SWATH.samples[samples_cols[[j]]] <- sum
}
# print(sums.SWATH.samples)
...
calc1 <- function(Intensity, Sum, MolWeight) {
  #print(Intensity)
  if (suppressWarnings(!is.na(as.double(Intensity)))) 
    return(as.double(Intensity)/(Sum * as.double(MolWeight))* MOL_CONVERSION) 
  else 
    return(NA)}# NULL

# for testing fixed example of the function
# print(calc1("12468.06", 12468.06, 2429640))

# apply calc1 to each number value of intensity in the input df and 
#  store the results and the df
# for each sample
for( j in 1:length(samples_cols)){  
  new_col_name <- paste(samples_cols[[j]], ".calc1", sep = "")
  print(new_col_name)
  
  temp <- rep(NA, df_swath_row_cn)
  # print(sums.SWATH.samples[[j]])
  ...
  for (i in 1:df_swath_row_cn) { # df_swath_row_cn
    ...
    # print(MW)
    sum <- sums.SWATH.samples[[j]]
    # apply the function and generate a temporary column
    temp[[i]] <- calc1(initial[[1]][i], sum, MW)
  } 
